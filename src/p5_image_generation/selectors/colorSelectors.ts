...
type ColorSchemeMap<T extends string> = {
  [K in T]: ColorScheme<K>
}

function createColorSchemeMap<T extends string>(
  schemes: ColorScheme<T>[]
): ColorSchemeMap<T> {
  return Object.fromEntries(
    schemes.map(scheme => [scheme.name, scheme])
  ) as ColorSchemeMap<T>;
}

export const COLOR_SCHEMES = createColorSchemeMap([
  {
    name: "DEBUG",
  ...
  {
    name: "Lava",
    colors: ["#342331", "#692F2E", "#D73F24", "#EC753A", "#F1E25F"]
  }
])
