...
import { evenSelector, everyOtherSelector, perlinSelector } from "./selectors/booleanValueSelectors";
import { DEBUG_COLOR_SCHEME, baseColorSelector, accentColorSelector, randomColorSelector } from "./selectors/colorSelectors";
import { randomOperationSelector } from "./selectors/shapeOperationSelector";

type PresetConfigGenerator = (p: p5, width: number, height: number) => () => LayerDispatcherConfig

const debugPreset1: PresetConfigGenerator = (p: p5, width: number, height: number) => {
...
const debugPreset2: PresetConfigGenerator = (p: p5, width: number, height: number) => {
